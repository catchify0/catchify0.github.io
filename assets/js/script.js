...
// Cache-Busting Live Badge Refresh for GitHub Stats
function refreshBadges() {
  const ts = Date.now();
  const downloadsBadge = document.getElementById("badge-downloads");
  const starsBadge = document.getElementById("badge-stars");
  const forksBadge = document.getElementById("badge-forks");

  if (downloadsBadge) {
    downloadsBadge.src =
      "https://img.shields.io/github/downloads/thamodharangm/catchify/total?style=flat-square&color=003258&labelColor=A6C8FF&label=DOWNLOADS&cacheSeconds=60&_=" +
      ts;
  }
  if (starsBadge) {
    starsBadge.src =
      "https://img.shields.io/github/stars/thamodharangm/catchify?style=flat-square&color=003258&labelColor=A6C8FF&label=STARS&cacheSeconds=60&_=" +
      ts;
  }
  if (forksBadge) {
    forksBadge.src =
      "https://img.shields.io/github/forks/thamodharangm/catchify?style=flat-square&color=003258&labelColor=A6C8FF&label=FORKS&cacheSeconds=60&_=" +
      ts;
  }
}

// Automatically refresh live stats on page load, tab focus, and after user downloads
document.addEventListener("DOMContentLoaded", function () {
  refreshBadges();

  // Attach instant refresh when user clicks any download button
  document.querySelectorAll("[data-download-link]").forEach((btn) => {
